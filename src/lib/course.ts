...
// Access can come from a purchase/manual enrolment OR an active subscription.
export async function fetchMyAccess(courseId: string) {
  const [enrollment, { data: subs }] = await Promise.all([
  ...
  const subscriptionActive =
    !!sub &&
    (["active", "trialing", "past_due"].includes(sub.status) || sub.status === "canceled") &&
    (!sub.current_period_end || new Date(sub.current_period_end) > new Date());
  return { enrollment, subscriptionActive, hasAccess: !!enrollment || subscriptionActive };
}
